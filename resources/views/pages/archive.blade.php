            ...
            <div class="container-flex space-between">
                <div class="authors-categories">
                    ...
                    <ul class="list-unstyled">
                        @foreach($authors as $author)
                            <li  class="text-uppercase c-green">{{ $author->name }}</li>
                        @endforeach
                    </ul>
                    ...
                    <ul class="list-unstyled">
                        @foreach($categories as $category)
                            <li class="text-capitalize">
                                <a class="text-uppercase c-green" href="{{ route('categories.show', $category)}}"> {{ $category->name}}</a></li>
                        @endforeach
                    </ul>
                </div>
                <div class="latest-posts">
                    ...
                    @foreach($posts as $post)
                        <p ><a class="text-uppercase c-green" href="{{ route('posts.show', $post)}}">{{ $post->title }}</a></p>
                    @endforeach
                    <h3 class="text-capitalize">Publicaciones por mes</h3>
                    <ul class="list-unstyled">
                        @foreach( $archive as $date)
                            <li><a class="text-uppercase c-green"  href="{{ route('pages.home', ['month' =>$date->month, 'year' => $date->year ] ) }}">
